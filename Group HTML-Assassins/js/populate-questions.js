$(function() {
    ...
    for (var key in questions) {
        ...
        var questionHtml = 
        `<section id='q${qId}' class='question-container ${question.display}'>
            <div id='question-q${qId}' class='question'><h3>#${qId}: ${question.question}</h3></div>
            <div data-bind="click: $root.selectAnswer" id='answer-one-q${qId}' class='answer'><p>${question.answerOne}</p></div>
            <div data-bind="click: $root.selectAnswer" id='answer-two-q${qId}' class='answer'><p>${question.answerTwo}</p></div>
            <div data-bind="click: $root.selectAnswer" id='answer-three-q${qId}' class='answer'><p>${question.answerThree}</p></div>
            <div data-bind="click: $root.selectAnswer" id='answer-four-q${qId}' class='answer'><p>${question.answerFour}</p></div>
            <div data-bind="click: $root.selectAnswer" id='correct-incorrect-q${qId}' class='question-result'></div>
        </section>`
